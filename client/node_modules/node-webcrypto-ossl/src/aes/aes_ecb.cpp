...
static Handle<std::string> AES_ECB_encrypt(Handle<std::string> hKey, Handle<std::string> hMsg, bool encrypt) {
	LOG_FUNC();
	...
	if (1 != EVP_CipherInit_ex(ctx.Get(), cipher, nullptr, key, NULL, encrypt)) {
...
Handle<std::string> ScopedAES::encryptEcb(Handle<std::string> hMsg) {
	LOG_FUNC();

	return AES_ECB_encrypt(this->value, hMsg, true);
}

Handle<std::string> ScopedAES::decryptEcb(Handle<std::string> hMsg) {
	LOG_FUNC();

	return AES_ECB_encrypt(this->value, hMsg, false);
}